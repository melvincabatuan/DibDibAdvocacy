        ...
        void inout_rect(const std::vector<cv::KeyPoint>& keypoints, cv::Point2f topleft, cv::Point2f bottomright, std::vector<cv::KeyPoint>& in, std::vector<cv::KeyPoint>& out)
        {
            for(unsigned int i = 0; i < keypoints.size(); i++)
            {
                if(keypoints[i].pt.x > topleft.x && keypoints[i].pt.y > topleft.y && keypoints[i].pt.x < bottomright.x && keypoints[i].pt.y < bottomright.y) 
                    in.push_back(keypoints[i]);    // foreground (tracked object)
                else out.push_back(keypoints[i]);  // background
            }
        }
...
*/
        void track(cv::Mat im_prev, cv::Mat im_gray, const std::vector<std::pair<cv::KeyPoint, int> >& keypointsIN, std::vector<std::pair<cv::KeyPoint, int> >& keypointsTracked, std::vector<unsigned char>& status, int THR_FB)// THR_FB - Forward-Backward Error Threshold
        {
            ...
            // This depends on both forward-backward LK Optical flow  
            status = std::vector<unsigned char>();
            
            //for(int i = 0; i < keypointsIN.size(); i++)
              //  status.push_back(false);
              
            //if at least one keypoint is active or detected
            if(keypointsIN.size() > 0)
            {
                ...
                //Initialise keypoints, pts, for forward optical flow 
                ...
                //Set status depending on forward-backward optical flow 
                for(unsigned int i = 0; i < status.size(); i++)
                    status[i] = (fb_err[i] <= THR_FB) & status[i];



				//Remember tracked keypoints 				
            	...
            	keypointsTracked = std::vector<std::pair<cv::KeyPoint, int> >(); // null
        }
...
*/
        ...
        CMT::CMT()
        {
            detectorType = "Feature2D.ORB";     // Default was FAST
            descriptorType = "Feature2D.ORB";   // Default was BRISK
            matcherType = "BruteForce-Hamming";
            //thrOutlier = 20;
            thrOutlier = 50;
            thrConf = 0.75;
            thrRatio = 0.8;
            descriptorLength = 512; // #TODO same with other desciptors?
            estimateScale = true;
            estimateRotation = true;
            nbInitialKeypoints = 0;


            detector = cv::ORB::create(500, 1.2f, 8, 31, 0, 2, cv::ORB::HARRIS_SCORE, 31, 10);

            // detector = cv::FastFeatureDetector::create(8);
            ...
            //detector = cv::AKAZE::create(); // slow and not accurate
         }
...
/** Initialise CMT with the ROI to be tracked */
        bool CMT::initialise(cv::Mat im_gray0, cv::Point2f topleft, cv::Point2f bottomright)
        {
            ...
            // Clear the trajectory on initialisation
            trajectory.clear();
            ...
            // These keypoints will be classified  as selected and background later
            std::vector<cv::KeyPoint> keypoints;
            detector->detect(im_gray0, keypoints);

            // Remember keypoints that are inside the rectangle as selected keypoints;
            // Otherwise, they are background keypoints
        	...
        	// Compute the keypoint descriptor for the selected keypoints
            ...
            if(selected_keypoints.size() == 0) // No keypoints found in selection
            {
                return false;
            }

            // Compute the keypoint descriptor for the background keypoints
            ...
            // Get all distances between selected keypoints  
            ...
            //Remember the roi rectangle corner coordinates relative to the center
            ...
            //Calculate dx, dy distances from each selected keypoints relative to the center
            ...
            /// Push the first center to trajectory
            trajectory.push_back(center);
            
            /// Push the first scale
            pseudoDepth.push_back(1.0);
        ...
        T median(std::vector<T> list)
        {
            ...
            if(list.size()%2 == 0) // even no. of elements
            {
        ...
        std::vector<Cluster> linkage(const std::vector<cv::Point2f>& list)
        {
            ...
            for(unsigned int i = 0; i < list.size(); i++)
            {
                ...
                {
                    if(i == j)
                        line.push_back(inf);           // Infinity
                    else
                    {
                        cv::Point2f p = list[i]-list[j];
                        line.push_back(sqrt(p.dot(p))); // Euclidean distance
                    }
                }
                for(unsigned int j = 0; j < list.size(); j++)
                    line.push_back(inf);
    ...
    */
        void CMT::estimate(const std::vector<std::pair<cv::KeyPoint, int> >& keypointsIN, cv::Point2f& center, float& scaleEstimate, float& medRot, std::vector<std::pair<cv::KeyPoint, int> >& keypoints)
        {
            ...
            // Initialise scale estimate
            scaleEstimate = NAN;
            
            // Initialise rotation estimate
            medRot = NAN;

            // at least 2 keypoints are needed for scale estimate
            if(keypointsIN.size() > 1)
            {
                ...
                if(ind1.size() > 0)
                {
                    ...
                    /// scale estimate, default 1
                    scaleEstimate = median(scaleChange);
                    if(!estimateScale)
                        scaleEstimate = 1;
                        
                    /// rotation estimate, default 0    
                    ...
                    // Segregate the keypoints with Consensus votes before
                    ...
                    /// Make the new keypoints the current keypoints
                    keypoints = newKeypoints;


					/// Compute the new center of the TRACKED object roi
        ...
        void CMT::processFrame(cv::Mat im_gray) // im_gray is the current gray frame
        {
        	// Track keypoints using Forward-Backward LK Optical flow
            trackedKeypoints = std::vector<std::pair<cv::KeyPoint, int> >();
            std::vector<unsigned char> status;
            track(im_prev, im_gray, activeKeypoints, trackedKeypoints, status);

			// Estimate the new tracked roi center, scale, and rotation 
            cv::Point2f center;
            float scaleEstimate;
            ...
            estimate(trackedKeypoints, center, scaleEstimate, rotationEstimate, trackedKeypoints2);
 			...
 			// Update the tracked keypoints to the consensus
            ...
            // Initialize list of active keypoints
            activeKeypoints = std::vector<std::pair<cv::KeyPoint, int> >();

            // Get the best two matches for each features in the database, 
            // containing both the selected and background descriptors
            // knnMatch computes the matches between the image descriptors
            ...
            // Get all matches for selected features if center exists
            if(!isnan(center.x) && !isnan(center.y))
                descriptorMatcher->knnMatch(features, selectedFeatures, selectedMatchesAll, selectedFeatures.rows);

			// Rotate the relative distances from the center with respect to the rotation estimate
            ...
            for(unsigned int i = 0; i < keypoints.size(); i++)
            {
                ...
                //Convert distances to confidences, do not apply weight
                ...
                // Note: Ratio Test filters 'good matches' by comparing the goodness 
                // of the best match between the goodness of the second best match. 
                // To be considered as a good match, this value must be higher than 
                // a certain ratio (e.x. 0.8 accdg. to Lowe), which can be set in a 
                // range of values between 0 and 1. If the ratio tends to be 0, 
                // the correspondence between descriptors is stronger. (e.x. ratio < thrRatio)
            ...
            if(!(isnan(center.x) | isnan(center.y)) && activeKeypoints.size() > nbInitialKeypoints / 10)
            {
                ...
                // Push the center to the trajectory
                trajectory.push_back(center);
                
                
                // Push scaleEstimate
				pseudoDepth.push_back(scaleEstimate);
